    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600 mt-1 capitalize">Your role: {membership.role}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          ...
          <p className="text-3xl font-bold text-slate-900">{org.material_lines?.length || 0}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          <p className="text-sm text-slate-500 mb-1">Page Views</p>
          <p className="text-3xl font-bold text-slate-900">{pageViews.toLocaleString()}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          <p className="text-sm text-slate-500 mb-1">Generations</p>
          <p className="text-3xl font-bold text-slate-900">{generationsStarted.toLocaleString()}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          <p className="text-sm text-slate-500 mb-1">Quote Requests</p>
          <p className="text-3xl font-bold text-slate-900">{quoteRequests.toLocaleString()}</p>
          <p className="text-xs text-slate-400 mt-1">{conversionRate}% conversion</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200">
          <h2 className="text-lg font-semibold text-slate-900">Material Lines</h2>
        </div>
        ...
        {org.material_lines && org.material_lines.length > 0 ? (
          <div className="divide-y divide-slate-100">
            {org.material_lines.map((materialLine) => (
              ...
              >
                <div>
                  <p className="font-medium text-slate-900">{materialLine.name}</p>
                  <p className="text-sm text-slate-500">
                    {materialLine.custom_domain && materialLine.custom_domain_verified
                      ? materialLine.custom_domain
                      : `${materialLine.slug}.${process.env.NEXT_PUBLIC_APP_DOMAIN || 'countertopvisualizer.com'}`}
                  </p>
                </div>
          ...
          <div className="px-6 py-12 text-center">
            ...
            <h3 className="text-lg font-medium text-slate-900 mb-2">No material lines yet</h3>
            <p className="text-slate-600 mb-6">Create your first material line to start using the visualizer.</p>
            <Link
              href={`/dashboard/organizations/${orgId}/material-lines/new`}
