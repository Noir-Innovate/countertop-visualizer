...
export default function ResultDisplay({
  ...
  const handleDownload = (imageData: string, slabName: string) => {
    ...
    link.download = `countertop-${slabName.toLowerCase().replace(/\s+/g, '-')}.png`
  ...
  const handleGetQuote = (imageId: string, imageName: string, imageUrl: string) => {
  ...
  return (
    <>
      <div className="card p-6 mb-8 animate-slide-up">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-[var(--color-text-secondary)] mt-1">
              {generationResults.length} {generationResults.length === 1 ? 'variation' : 'variations'} generated
            </p>
          </div>
          {allComplete && (
            <button
        ...
        {/* Comparison View */}
        {viewMode === 'compare' && hasCompletedImages && allImages.length >= 2 && (
          <div className="mb-6">
            <ImageComparison
              images={allImages}
              ...
              <div key={result.slabId} className="space-y-3">
                ...
                {result.isLoading && (
                  <div className="aspect-video bg-[var(--color-bg-secondary)] rounded-lg flex items-center justify-center">
                    <div className="text-center">
                      <svg className="animate-spin h-8 w-8 text-[var(--color-accent)] mx-auto mb-2" viewBox="0 0 24 24">
                        <circle
                ...
                {result.error && (
                  <div className="aspect-video bg-red-50 rounded-lg flex items-center justify-center p-4">
                    <p className="text-sm text-red-600 text-center">{result.error}</p>
                  </div>
                )}
              </div>
            ))}
