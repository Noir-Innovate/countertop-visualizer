...
export default function ImageCarousel({
  ...
  const trackMaterialViewed = useCallback(
    (index: number) => {
      ...
      if (!currentImage || currentImage.isOriginal || currentImage.isLoading) {
        return;
      }

      // Find material type from selectedSlabs
      const slab = selectedSlabs.find((s) => s.id === currentImage.id);
      const materialType = slab?.material_type || null;
  ...
  const handleIndexChange = useCallback(
    (newIndex: number) => {
      ...
      trackMaterialViewed(newIndex);
    },
    [onIndexChange, trackMaterialViewed],
  );
