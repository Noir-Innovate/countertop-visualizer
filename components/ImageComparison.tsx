...
export default function ImageComparison({
  ...
  const trackMaterialViewed = useCallback(() => {
    // Skip tracking if either image is original or loading
    if (
    ...
    // Find material types from selectedSlabs
    const leftSlab = selectedSlabs.find((s) => s.id === leftImage.id);
    const rightSlab = selectedSlabs.find((s) => s.id === rightImage.id);
    const leftMaterialType = leftSlab?.material_type || null;
    const rightMaterialType = rightSlab?.material_type || null;
  ...
  // Track when indices change
  useEffect(() => {
    trackMaterialViewed();
  }, [leftIndex, rightIndex, trackMaterialViewed]);

  const handleLeftIndexChange = useCallback(
    (index: number) => {
  ...
  const handleRightIndexChange = useCallback(
    (index: number) => {
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="absolute inset-0">
          ...
          {!rightIsLoading && (
            <div
              className={`absolute top-3 right-3 px-3 py-1 text-white text-sm font-medium rounded-full z-10 ${
                rightImage.isOriginal
        ...
        >
          ...
          {!leftIsLoading && (
            <div
              className={`absolute top-3 left-3 px-3 py-1 text-white text-sm font-medium rounded-full z-10 ${
                leftImage.isOriginal
                  ? "bg-[var(--color-primary)]"
                  : "bg-[var(--color-accent)]"
              }`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 px-2 md:px-0">
        ...
        <div className="space-y-2">
          ...
          <select
            value={leftIndex}
            onChange={(e) => handleLeftIndexChange(Number(e.target.value))}
        ...
        <div className="space-y-2">
          ...
          <select
            value={rightIndex}
            onChange={(e) => handleRightIndexChange(Number(e.target.value))}
            className="w-full px-3 py-2 rounded-lg border border-[var(--color-border)] bg-white text-[var(--color-text)] focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)]"
          >
            {images.map((img, idx) => (
              <option key={img.id} value={idx}>
