...
/** Default prompt used when DB is unavailable or for local testbed. Exported for scripts. */
export const DEFAULT_TAKEOFF_PROMPT = `**Role:** You are a professional countertop fabricator, estimator, and draftsman. 
**Task:**
...
export interface TakeoffPromptResult {
  content: string;
  versionId: string | null;
}

/**
 * Fetches the latest takeoff prompt version from Supabase (service role only).
 * Returns content and version id so the caller can record which version was used.
 * Falls back to bundled default if DB is unavailable or no version exists.
 */
export async function getTakeoffPrompt(): Promise<TakeoffPromptResult> {
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY;

  if (!url || !serviceRoleKey) {
    return { content: DEFAULT_TAKEOFF_PROMPT, versionId: null };
  }
