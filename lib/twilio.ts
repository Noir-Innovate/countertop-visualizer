...
function getVerifyServiceSid(): string {
  ...
  if (!serviceSid) {
    throw new Error('TWILIO_VERIFY_SERVICE_SID not configured. Create a Verify service in Twilio Console and add the Service SID.')
  }
...
export async function sendVerificationCode(phone: string): Promise<{ success: boolean; error?: string; sid?: string }> {
  try {
    const client = getTwilioClient()
...
export async function verifyCode(phone: string, code: string): Promise<{ success: boolean; error?: string; status?: string }> {
  try {
    const client = getTwilioClient()
    const serviceSid = getVerifyServiceSid()
