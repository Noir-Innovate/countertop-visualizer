...
export async function getSlabsForMaterialLine(
  ...
  let materialsMap: Map<string, { title: string | null; material_type: string | null; order: number }> = new Map();
  
  if (materialLineId) {
    const { data: materials } = await supabase
      .from("materials")
      .select("filename, title, material_type, order")
  ...
  // Return slabs without the order and filename fields
  const slabs: Slab[] = slabsWithOrder.map(({ order, filename, ...slab }) => slab);
