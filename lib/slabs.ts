...
export interface Slab {
  ...
  name: string;
  imageUrl: string;
  description: string; // Keep for backwards compatibility but will use material_type
...
export async function getSlabsForMaterialLine(
  ...
  if (materialLineId) {
    const { data: materials } = await supabase
      .from("materials")
      .select("filename, title, material_type, order, price_per_sqft")
  ...
  // Filter for image files and map to Slab objects with order info
  const slabsWithOrder = (files || [])
    ...
    .map((file) => {
      ...
      return {
        ...
        description: autoGeneratedDescription, // Keep for backwards compatibility
        material_type: materialMeta?.material_type || null,
        price_per_sqft: materialMeta?.price_per_sqft ?? null,
        order: materialMeta?.order ?? 999999, // Put materials without order at the end
        filename: file.name,
      };
    })
    ...
    .sort((a, b) => {
      ...
      // Fallback to filename alphabetical if order is the same
      return a.filename.localeCompare(b.filename);
    });

  // Return slabs without internal order field; keep filename for designer/material matching
  const slabs: Slab[] = slabsWithOrder.map(({ order, ...slab }) => slab);
